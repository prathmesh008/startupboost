...
We will deploy the `platform_engine` (Node.js/Express) to Render.
...
3.  **Configure Service**:
    *   **Root Directory**: `platform_engine` (IMPORTANT)
    *   **Runtime**: Node
...
We will deploy the `webapp_client` (Next.js) to Vercel.
...
2.  **Configure Project**:
    *   **Root Directory**: Click the "Edit" button next to "Root Directory" and select `webapp_client`.
...
3.  If you get a Network Error:
    ...
    *   **Quick Fix for CORS**: You might need to update `platform_engine/nucleus.ts` to allow all origins `origin: '*'` temporarily or add your specific Vercel domain.

### Allowing Vercel Domain in Backend
If you face CORS issues, update `nucleus.ts`:
```typescript
app.use(cors({
    origin: [
        'http://localhost:3000', 
